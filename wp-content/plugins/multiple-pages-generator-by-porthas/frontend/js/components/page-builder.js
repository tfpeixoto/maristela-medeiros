...
jQuery('select[name="periodicity"]').on('change', function () {
    ...
    if (value !== 'now') {
        // Делаем остальные поля доустпными.
...
jQuery('#mpg_entity_type_dropdown').on('change', async function () {
...
jQuery('.project-builder section[data-id="2"] .save-changes').on('click', async function () {
    ...
    if (directLink && periodicity && periodicity !== 'now') {
...
jQuery('#mpg_unschedule_task').on('click', async function () {
    ...
    if (decision) {
        ...
        setTimeout(() => {
            location.href = `${backendData.projectPage}&id=${mpgGetState('projectId')}`;
        }, 1000);
    }
})
...
jQuery('.main-template-info').on('submit', async function (e) {
    ...
    setTimeout(() => {
        location.href = `${backendData.projectPage}&id=${projectId}`;
    }, 1000);

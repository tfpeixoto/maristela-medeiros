...
class MPG_Helper
{
    ...
    public static function mpg_send_analytics_data()
    {
        try {
            ...
            $mpg_track_id = sanitize_text_field($_POST['mpg_track_id']);


            if (strlen($mpg_track_id) !== 0) {

                $current_user = wp_get_current_user();

                wp_remote_get('https://www.mpgwp.com/wp-json/cookie/link?mpg-track-id=' . $mpg_track_id . '&user_email=' . $current_user->user_email);
    ...
    public static function mpg_front_assets_enqueue()
    {
        ...
        if (is_search()) {
            wp_enqueue_script('mpg_searchpage', plugins_url('multiple-pages-generator-by-porthas/frontend/js/mpg-front-search.js'),  array('jquery'));
...
<?php }
    ...
    public static function mpg_prepare_post_excerpt($short_codes, $strings, $post_content)
    {
        ...
        $string = wp_trim_words($string, (int) get_option('mpg_search_excerpt_legnth'));

...
spl_autoload_register(function ($class_name) {
    ...
    for ($i = count($file_parts) - 1; $i > 0; $i--) {
        ...
        if (count($file_parts) - 1 === $i) {
             ...
             * filename structure.
             */
            if (strpos(strtolower($file_parts[count($file_parts) - 1]),
                'interface')
            ) {

                // Grab the name of the interface from its qualified name.
                $interface_name = explode('_',
    ...
    $filepath
        = trailingslashit(untrailingslashit(plugin_dir_path(dirname(__DIR__)))
                          . $namespace);
    $filepath .= $file_name;
    ...
    if (file_exists($filepath)) {
        ...
        wp_die(
            esc_html('The file attempting to be loaded at ' . $filepath
                     . ' does not exist.')
        );
    }
}

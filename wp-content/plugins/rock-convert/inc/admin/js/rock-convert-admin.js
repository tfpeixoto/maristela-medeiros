            ...
            $(this).on('click', 'a', function (e) {
                ...
                if (!isRunning) {
                    ...
                    $.ajax({
                        ...
                        beforeSend: function () {
                            $('body.wp-admin').addClass('rock-ajax-request-load');
                        }
                    }).success(function (data) {
                        $('body').append(data);
                    }).complete(function () {

                        $('body.wp-admin').removeClass('rock-ajax-request-load');
                        isRunning = false;
                        let delete_actions = $('.delete-actions');
                        delete_actions.on('click', '.cancel-action', function (e) {
                            e.preventDefault();
                            $('.rock-confirm-delete-lead ~ span').remove();
                            $('.rock-confirm-delete-lead').remove();
                        });

                        delete_actions.on('click', '.confirm-action', function (e) {
                            e.preventDefault();
                            let entry_id = $(this).attr('data-entry');
                            if (!isRunning) {
                                isRunning = true;
